...
- Migrations live in `migrations/`. **Always** produce them with `bun run db:generate` from this package (`package.json` script)—do not hand-write SQL, `_journal.json`, or snapshot files. Drizzle needs the SST secret `DatabaseUrl`: run via `sst shell -- bun run --cwd packages/core db:generate` (and `db:migrate` the same way), or set `DATABASE_URL` locally.
