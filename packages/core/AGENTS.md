...
- Example EventBridge event: `src/template-event` + subscriber in `packages/functions/src/event`.
- Example CRUD slice: `src/demo` (`demo_item` table) → `packages/functions/src/api/demo.ts` → `@repo/sdk` `DemoResource` → `apps/app` route `/demo`.
